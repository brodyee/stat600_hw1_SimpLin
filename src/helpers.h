...
arma::mat getStdErr(const arma::vec& residuals, const arma::mat& X) {
  double varHat = ((X.n_rows - 1)/(X.n_rows - X.n_cols))*arma::var(residuals);
  arma::mat sigHat = arma::sqrt(varHat * arma::inv(X.t() * X));
...
arma::mat getConfInt(const arma::vec& beta, const arma::mat& stdErr,
  ...
  for (int i = 0; i < NUM_COEF; i++) {
    confInt.at(i, 0) = beta[i] 
                    - R::qt(1 - alpha / 2, n - beta.n_elem, true, false) 
                      * stdErr.at(i, i);
    confInt.at(i, 1) = beta[i]
                    + R::qt(1 - alpha / 2, n - beta.n_elem, true, false) 
                      * stdErr.at(i, i);
  }
